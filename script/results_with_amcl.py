...
def plot_trajectory_stages(data, map_data=None, map_origin=(-10, -10), map_resolution=0.05, title=None):
    ...
    real_states = np.array(data['real_states']) if 'real_states' in data else None
    ...
    for i, idx in enumerate(indices):
        ...
        if i == 0:
            ax.plot(states[:, 0], states[:, 1], 'b-', linewidth=1, label='AMCL Estimated Trajectory')
            # Also plot real states if available
            if real_states is not None:
                ax.plot(real_states[:, 0], real_states[:, 1], 'g--', linewidth=1, label='Ground Truth Trajectory')
        else:
            # For other figures, plot trajectory up to the specific stage
            ax.plot(states[:idx+1, 0], states[:idx+1, 1], 'b-', linewidth=2, label='AMCL Estimated Trajectory')
            if real_states is not None and i > 0:
                ax.plot(real_states[:idx+1, 0], real_states[:idx+1, 1], 'g--', linewidth=2, label='Ground Truth Trajectory')
        
        # Plot the start point with orientation arrow
        start_x, start_y, start_theta = states[0]
        ax.plot(start_x, start_y, 'bo', markersize=6, label='AMCL Start')
        # Add arrow to show orientation at start
        arrow_length = 0.3
        dx = arrow_length * np.cos(start_theta)
        dy = arrow_length * np.sin(start_theta)
        ax.arrow(start_x, start_y, dx, dy, head_width=0.1, head_length=0.15, fc='b', ec='b')
        
        # Plot real start if available
        if real_states is not None:
            real_start_x, real_start_y, real_start_theta = real_states[0]
            ax.plot(real_start_x, real_start_y, 'go', markersize=6, label='Ground Truth Start')
            # Add arrow to show orientation at real start
            dx = arrow_length * np.cos(real_start_theta)
            dy = arrow_length * np.sin(real_start_theta)
            ax.arrow(real_start_x, real_start_y, dx, dy, head_width=0.1, head_length=0.15, fc='g', ec='g')
        
        # Plot the current point with orientation arrow
        current_x, current_y, current_theta = states[idx]
        ax.plot(current_x, current_y, 'ro', markersize=6, label='AMCL Current Position')
        ...
        # Plot real current position if available
        if real_states is not None:
            real_current_x, real_current_y, real_current_theta = real_states[idx]
            ax.plot(real_current_x, real_current_y, 'mo', markersize=6, label='Ground Truth Current Position')
            # Add arrow to show real current orientation
            dx = arrow_length * np.cos(real_current_theta)
            dy = arrow_length * np.sin(real_current_theta)
            ax.arrow(real_current_x, real_current_y, dx, dy, head_width=0.1, head_length=0.15, fc='m', ec='m')
        ...
        ax.plot(goal_x, goal_y, 'yx', markersize=10, label='Goal')
        # Add arrow to show goal orientation
        dx = arrow_length * np.cos(goal_theta)
        dy = arrow_length * np.sin(goal_theta)
        ax.arrow(goal_x, goal_y, dx, dy, head_width=0.1, head_length=0.15, fc='y', ec='y')
        ...
        if i == 0:
            ax.set_title(f'Triton Trajectory with AMCL')
        else:
...
def main():
    ...
    pickle_file = glob(os.path.join(results_dir, "trajectory_data_with_amcl_1.pkl"))
    ...
    if save_plots:
        ...
        plots_dir = os.path.join(results_dir, "plots_with_amcl")
    ...
    print(f"Final state (AMCL): {final_state}")
    print(f"Goal state: {goal}")
    print(f"Final distance to goal: {distance_to_goal:.3f} meters")
    
    # If real states are available, calculate the final real distance to goal
    if 'real_states' in data and data['real_states']:
        final_real_state = np.array(data['real_states'][-1])
        real_distance_to_goal = np.sqrt((final_real_state[0] - goal[0])**2 + (final_real_state[1] - goal[1])**2)
        
        print(f"Final real state (Ground Truth): {final_real_state}")
        print(f"Final real distance to goal: {real_distance_to_goal:.3f} meters")
        
        # Calculate AMCL estimation error
        estimation_error = np.sqrt((final_state[0] - final_real_state[0])**2 + 
                                   (final_state[1] - final_real_state[1])**2)
        print(f"Final AMCL estimation error: {estimation_error:.3f} meters")
